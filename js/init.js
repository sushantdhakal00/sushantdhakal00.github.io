...
if($('#slide-out-widget-area.slide-out-from-right-hover').length > 0) {
	...
	if($('#bullmonk-wrap > .slide-out-widget-area-toggle').length == 0) {
		$('<div class="slide-out-widget-area-toggle slide-out-hover-icon-effect" data-icon-animation="simple-transform"><div> <a href="#sidewidgetarea" class="closed"> <span> <i class="lines-button x2"> <i class="lines"></i> </i> </span> </a> </div> </div>').insertAfter('#slide-out-widget-area');
		if($('#header-outer[data-has-menu="true"]').length > 0 || $('body[data-header-search="true"]').length > 0) $('#bullmonk-wrap > .slide-out-widget-area-toggle').addClass('small');
	}
...
$('body').on('click','.slide-out-widget-area-toggle:not(.std-menu) a.closed:not(.animating)',function(){
	...
	if($('#slide-out-widget-area').hasClass('slide-out-from-right')) {
		...
		if($('.mobile #header-outer[data-permanent-transparent="false"]').length > 0 && $('.container-wrap').hasClass('no-scroll')) $('#bullmonk-wrap').addClass('at-content');
...
$('body').on('click','.slide-out-widget-area-toggle:not(.std-menu) a.open:not(.animating), #slide-out-widget-area .slide_out_area_close, #slide-out-widget-area-bg.slide-out-from-right',function(){
	...
	if($('#slide-out-widget-area').hasClass('slide-out-from-right')) {
		...
		$('#slide-out-widget-area-bg').stop(true).transition({'opacity': 0},900,'easeOutQuad',function(){
			if($('.mobile #header-outer[data-permanent-transparent="false"]').length > 0 && $('.container-wrap').hasClass('no-scroll')) $('#bullmonk-wrap').removeClass('at-content');
...
function pageHeader(){
	...
	//handle left header 
	if($('body[data-header-format="left-header"]').length > 0 ) $('#page-header-bg[data-parallax="1"]').css('width',$('#bullmonk-wrap').width())
...
function pageHeaderInit(){
	 ...
	 //transparent
	  ...
	  //fade in header
	  if($('#bullmonk-wrap').length == 0 || !$('body').hasClass('ajax-loaded')){
	  	$('#page-header-bg[data-parallax="1"]').animate({ 'opacity' : 1},650,'easeInCubic');
	  } else if($('#bullmonk-wrap').length == 1) {
...
function boxRollInit() {
	if($('.nectar-box-roll').length > 0) { 
		...
		$('body, html, #bullmonk-wrap, .container-wrap, .blurred-wrap').addClass('no-scroll');
		...
		$('#bullmonk-wrap, .blurred-wrap').addClass('at-content');
		$('body, html, #bullmonk-wrap, .container-wrap, .blurred-wrap').removeClass('no-scroll');
...
function nectarBoxRollContentHeight() {
	...
	if($('#header-outer[data-transparent-header="true"]').length == 0) {
		...
		if($('.mobile').length == 0 && $('body[data-header-format="left-header"]').length == 0) { $('#bullmonk-wrap').css('margin-top',$headerNavSpace); $('#slide-out-widget-area.fullscreen').css('margin-top','-'+$headerNavSpace+'px'); }
		else $('#bullmonk-wrap, #slide-out-widget-area.fullscreen').css('margin-top','0');
	} else {
...
function boxRoll(e,d) {
	...
	if(perspect == 'not-rolled' && animating == 'false' && d == -1) {
		...
		if($('#header-outer[data-permanent-transparent="1"]').length == 0 && $('.mobile').length == 0 && $('#header-outer[data-transparent-header="true"]').length != 0) { 
			$('#bullmonk-wrap').transition({'margin-top':$('#header-outer').outerHeight(true) + $('#header-outer').offset().top},2000,'easeInOutQuad');
		}
		/*if($('#header-outer[data-transparent-header="true"]').length == 0 && $('.mobile').length == 0) {
		...
		setTimeout(function(){
			...
			//header position when transparent nav was used
			if($('#header-outer[data-transparent-header="true"]').length != 0) { 
				$('#bullmonk-wrap').stop(true,true).transition({'margin-top':0},2000,'easeInOutCubic');
			} else {
		...
		$('body,html,#bullmonk-wrap, .container-wrap, .blurred-wrap').addClass('no-scroll');
		$('#bullmonk-wrap, .blurred-wrap').removeClass('at-content');
...
if(navigator.userAgent.match(/(Android|iPod|iPhone|iPad|BlackBerry|IEMobile|Opera Mini)/) && $('.nectar-box-roll').length > 0) {
	$('body').swipe({
		...
		swipeStatus: function(event, phase, direction, distance, duration, fingers) {
			...
			if(direction == 'up') {
				boxRoll(null,-1);
				if($('#bullmonk-wrap.no-scroll').length == 0) $('body').swipe("option", "allowPageScroll", 'vertical');
...
/***************** Nav ******************/
	...
	function smallNav(){
		...
		if($offset > 0 && $windowWidth > 1000) {
			...
			//box roll
			if($('.nectar-box-roll').length > 0 && $('#header-outer[data-permanent-transparent="1"]').length == 0) $('#bullmonk-wrap').animate({'margin-top':  (Math.floor((logoHeight-shrinkNum) +(headerPadding*2)/ 1.8 + extraHeight + secondaryHeader))  },{queue:false, duration:250, easing: 'easeOutCubic'})
	...
	function bigNav(){
		...
		if($offset == 0 && $windowWidth > 1000 || $('.small-nav').length > 0 && $('#bullmonk-wrap.no-scroll').length > 0 || $('.small-nav').length > 0 && $('.slide-out-from-right-hover.open').length > 0 || $('.small-nav').length > 0 && $('.slide-out-from-right.open').length > 0 && $bodyBorderHeaderColorMatch == true) {
			...
			if(usingLogoImage == false) $('header#top #logo').stop(true,true).animate({
				...
				//box roll
			if($('.nectar-box-roll').length > 0 && $('#header-outer[data-permanent-transparent="1"]').length == 0) $('#bullmonk-wrap').animate({'margin-top':  (Math.floor((logoHeight) +(headerPadding*2) + extraHeight + secondaryHeader))  },{queue:false, duration:250, easing: 'easeOutCubic'})
...
/*-------------------------------------------------------------------------*/	
	...
	function pinterestShare(){
		windowLocation = window.location.href.replace(window.location.hash, '');
		var $sharingImg = ($('.single-portfolio').length > 0 && $('div[data-featured-img]').attr('data-featured-img') != 'empty' ) ? $('div[data-featured-img]').attr('data-featured-img') : $('#bullmonk-wrap img').first().attr('src'); 
...
if($('body[data-fancy-form-rcs="1"]').length > 0) {
	...
	if($('body[data-ajax-transitions="true"]').length > 0 && $('#ajax-loading-screen[data-method="ajax"]').length > 0 && !navigator.userAgent.match(/(Android|iPod|iPhone|iPad|BlackBerry|IEMobile|Opera Mini)/) && $(window).width() > 690 ) {
		...
		$('#bullmonk-wrap').ajaxify({
			'selector':'#bullmonk-wrap a:not(.no-ajaxy):not([target="_blank"]):not([href^="#"]):not(.comment-edit-link):not(#cancel-comment-reply-link):not(.comment-reply-link):not(#toggle-nav):not(.cart_list a):not(.logged-in-as a):not(.no-widget-added a):not(.add_to_cart_button):not(.product-wrap a):not(.section-down-arrow):not([data-filter]):not(.product_list_widget a):not(.pp):not([rel^="prettyPhoto"]):not(.pretty_photo), #header-outer li:not(.no-ajaxy) > a:not(.no-ajaxy), #header-outer #logo',
			'verbosity': 0, 
			requestDelay: 400,
			previewoff : true,
			memoryoff: true,
